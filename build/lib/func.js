...
class MyFunc {
     ...
     * @returns
     */
    isLoginValid(loginData, now = new Date()) {
        if (loginData != null) {
     ...
     */
    isACLoading(value) {
        return Object.values(apitypes_1.ACLoadCodes).includes(value);
    }
    /**
            ...
            for (let i = 1; i < sorted.length; i++) {
                ...
                if (curr.start < prev.end) {
                    ...
                    const returntext = errortext
                        ...
                        */
                    const returntext = this.format(errortext, day.toString(), this.formatTime(curr.start), this.formatTime(prev.end));
                    return returntext;
                }
            }
        }
        ...
        const cleanse = (input) => {
            ...
            if (typeof input === 'object' && input !== null) {
                ...
                for (const [sectionKey, arrayKeys] of Object.entries(rules.conditional ?? {})) {
                    ...
                    const shouldRemove = arrayKeys.every((arrayKey) => {
                        const value = section[arrayKey];
    ...
    format(template, ...args) {
        return template.replace(/%(?:(\d*\.\d+)f|s(\d*)|d(\d*)|b(\d*)|t(\d*)\(([^)]+)\)|T(\d*)\(([^)]+)\))/g, (match, floatFmt, sIndex, dIndex, bIndex, tIndex, tFormat, TIndex, TFormat) => {
